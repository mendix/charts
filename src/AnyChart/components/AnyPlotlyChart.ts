...
import { Config, Data, Layout, PieData, PieHoverData, ScatterData, ScatterHoverData, newPlot, purge } from "plotly.js/dist/plotly";
// TODO check if code base could be shared with other chart widgets, by overwriting alias lookup for any chart
...
export class PlotlyChart extends Component<PlotlyChartProps, {}> {
    ...
    private renderChart({ config, data, layout, onClick, onHover }: PlotlyChartProps) {
        if (this.chartNode) {
            ...
            console.log("Plotly.newPlot('myDiv'," + JSON.stringify(data, null, 2) + " , " + JSON.stringify(layoutOptions, null, 2) + ", " + JSON.stringify(config, null, 2) + ");");
